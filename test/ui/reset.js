...
describe('Resetting UI State', () => {
  ...
  describe('on a UI component with no children', () => {
    it('resets its own UI state', () => {
  ...
  describe('on a child UI', () => {
    it('resets only its own context', () => {
  ...
  describe('on a parent UI', () => {
    it('resets its own context AND its children\'s contexts', () => {
        const tree = render(<UIParent><UIChild /></UIParent>);
        const parent = ReactTestUtils.findRenderedComponentWithType(tree, Parent);
        const child = ReactTestUtils.findRenderedComponentWithType(tree, Child);
  ...
  describe('with a child that has evaluated state', () => {
      it('works', () => {
        const FunctionalUIChild = ui({
          state: {
            evaluated: (props) => props.value
